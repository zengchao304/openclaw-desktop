...
2. 运行安装程序（文件名与 `package.json` 一致，例如 `OpenClaw-Setup-0.6.3+openclaw.2026.3.31.exe`）
...
- **Shell 版本：** `0.6.3+openclaw.2026.3.31`（主版本号 + 构建元数据中的捆绑 OpenClaw 版本）。
- **Git 发行标签：** `v0.6.3+openclaw.2026.3.31` — 与根目录 `package.json` 的 `version` 加前缀 `v` 一致，标签内可见捆绑 OpenClaw 版本号。
...
- **桌面端近期要点：** **v0.6.3** 强化**内嵌 Control UI** 配置：`openclaw.json` 缺少 `gateway` 时也会写入 **2026.3.x** 所需 iframe 标志；所有 **`writeOpenClawConfig`** 写盘路径都会合并上述字段；写盘**短暂重试**缓解 Windows 偶发锁文件导致迁移未落盘。详见 [CHANGELOG **0.6.3**](CHANGELOG.md)。**v0.6.2** 约定 **Git 标签携带捆绑 OpenClaw 版本**（如 `v0.6.2+openclaw.2026.3.31`，[CHANGELOG **0.6.2**](CHANGELOG.md)）。**v0.6.1** 修复 Control UI **500 / device identity**（[**0.6.1**](CHANGELOG.md)）。**v0.6.0** 修复 stub `package.json` / `prepare-bundle`（[**0.6.0**](CHANGELOG.md)）。飞书 `registerFull` 防护；MiniMax **仅 M2.7**；Control UI 自 GitHub 标签构建。
...
| | |
|---|---|
| **Git 标签 / 壳版本** | **`v0.6.3+openclaw.2026.3.31`**（即 `v` + `package.json` 的 `version`） |
| **安装包** | `OpenClaw-Setup-0.6.3+openclaw.2026.3.31.exe`（见 [Releases](https://github.com/agentkernel/openclaw-desktop/releases/latest) 实际资产名） |
...
**捆绑 OpenClaw：** 在 `package.json` 中通过 `openclawBundleVersion` 固定；执行 `prepare-bundle` 后查看 [`resources/bundle-manifest.json`](resources/bundle-manifest.json) 中的 `bundledOpenClawVersion`（桌面 **v0.6.3** 当前为 **2026.3.31**）。本地校验：`pnpm run check-openclaw-versions`（不设 `OPENCLAW_SKIP_NPM_LATEST_CHECK` 时还会与 npm `latest` 对比）。
