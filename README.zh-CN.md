...
<details>
<summary><strong>如何诊断 Enterprise Shell 运行时注入是否生效？</strong></summary>

桌面端现在会在拉起 bundled gateway 前检查当前用户的 Enterprise Shell manifest：`%LOCALAPPDATA%\OpenClawEnterpriseShell\support\install-manifest.json`。当 manifest 完整有效时，Desktop 会记录 `active` enterprise launch 状态，并显式把 `-r <decrypt-loader>` 与 `--import <esm-bootstrap>` 注入 gateway child。

如果 Enterprise Shell 缺失或损坏，Desktop 会 fail-open 回退到原生启动链。可通过 `%USERPROFILE%\.openclaw\logs\shell.log` 或执行 `pnpm run verify-bundle` 区分以下情况：

- 未发现 manifest
- manifest 已发现，但字段无效或 runtime 资产缺失
- enterprise runtime launch 已激活
</details>
